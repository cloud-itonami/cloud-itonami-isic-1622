(ns millwork.operation
  "MillworkOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MillworkAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Millwork Shop Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [millwork.advisor :as advisor]
            [millwork.governor :as governor]
            [millwork.phase :as phase]
...
(defn build
  "Compiles a MillworkOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `millwork.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MillworkAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Millwork Shop Plant Operations Governor -- independent censor.
      (g/add-node :govern
