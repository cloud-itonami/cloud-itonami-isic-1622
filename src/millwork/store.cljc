(ns millwork.store
  "SSoT for the millwork-shop plant-operations coordination actor,
  ...
  Decision 1, this vertical is self-contained (no external millwork
  capability library, no jurisdiction-scoped Datomic-parity
  requirement driving a second backend); a `langchain.db`-backed
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             ...
                             record (doors, window frames, staircases,
                             ...
                             ground truth.
    - `equipment`         -- a cutting/joinery/finishing-line unit's
                             own record (panel saw, CNC router,
                             tenoning machine, edge bander, finishing
    ...
    - `maintenance`       -- a scheduled cutting/joinery/finishing-line
                             maintenance-window DRAFT against a piece
                             of equipment (`millwork.registry`'s
    ...
    - `shipments`         -- a proposed outbound-millwork-shipment
                             DRAFT (`millwork.registry`'s
                             `register-shipment`).
  ...
  audit trail a shop owner or millwork buyer trusting this coordinator
  needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (millwork.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (millwork.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :dimensional-spec :door-single-leaf :product-type :door
                :unit-count 500 :output-quality-percent 96.0
                :verified? true :registered? true
                :shipped-unit-count 100
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :dimensional-spec :window-casement :product-type :window-frame
                :unit-count 80 :output-quality-percent 92.0
                :verified? true :registered? true
                :shipped-unit-count 75
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :dimensional-spec :staircase-straight-flight :product-type :staircase
...
(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :cnc-router
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :tenoning-machine
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `millwork.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `millwork.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  shipment coordinated against it); one verified+registered CNC-router
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  tenoning-machine unit (blocks any maintenance scheduling against
