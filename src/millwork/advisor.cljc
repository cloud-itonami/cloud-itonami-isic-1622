(ns millwork.advisor
  "MillworkAdvisor -- the *contained intelligence node* for the
  millwork-shop plant-operations coordination actor.

  It normalizes production-batch patches (dimensional-spec/unit-count/
  output-quality for doors, window frames, staircases and structural
  wood components), drafts a cutting/joinery/finishing-equipment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound-millwork-shipment
  ...
  a real cutting/joinery/finishing-line-equipment actuation, cutting-
  line-run execution, or freight dispatch. Every output is censored
  downstream by `millwork.governor` before anything touches the SSoT
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; control or cutting-line-finalize
                                 ; effect
  ...
  contract every caller of this actor agrees to) -- `millwork.governor`
...
(defn- schedule-maintenance
  "Draft a cutting/joinery/finishing-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `millwork.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft an equipment-safety/materials-safety/dust-hazard
  ...
  administrative technicality). See `millwork.phase`: no phase ever
  adds this op to a phase's `:auto` set; `millwork.governor` also
...
(defn- coordinate-shipment
  "Draft an outbound-millwork-shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed unit count -- it does NOT invent one, and `millwork.governor`
  NEVER trusts it: it independently recomputes whether the batch's own
  cumulative-shipped unit count plus this claim would exceed the
  batch's own recorded unit count before any commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは建具・造作(ミルワーク)工場プラント運用コーディネーターの助言者です。"
       ...
       "切断・接合・仕上げラインの稼働確定(finalize)や設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `millwork.governor` escalates/
  holds -- an LLM hiccup can never auto-schedule maintenance, auto-flag
  a concern, or auto-coordinate a shipment."
  [content]
