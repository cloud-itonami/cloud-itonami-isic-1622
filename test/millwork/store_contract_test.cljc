(ns millwork.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `millwork.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:dimensional-spec :door-double-leaf}})
    (is (= :door-double-leaf (:dimensional-spec (store/batch s "batch-001"))))
